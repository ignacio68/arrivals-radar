...
export const mbSetViewport = (map: MapboxViewApi, options: SetViewportOptions): void => {
  console.log('___mbSetViewport()')
  map.setViewport(options).then((result) => console.log(`mbSetViewport: ${JSON.stringify(result)}`))
}

export const mbGetViewport = (map: MapboxViewApi): void => {
  map.getViewport().then((result) => console.log(`mbGetViewport: ${JSON.stringify(result)})`))
}
